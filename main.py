        ...
        for i in range(count - 1, len(links)):
            ...
            try:  
                ...
                if "drive" in url:
                    ...
                    try:
                        cmd = f'yt-dlp -o "❤️ @SONICKUWALSSCBOT ❤️{name}.pdf" "{url}"'
                        download_cmd = f"{cmd} -R 25 --fragment-retries 25"
                        os.system(download_cmd)
                        copy = await bot.send_document(chat_id=m.chat.id, document=f'❤️ @SONICKUWALSSCBOT ❤️{name}.pdf', caption=cc1)
                        count += 1
                        os.remove(f'❤️ @SONICKUWALSSCBOT ❤️{name}.pdf')
                    except FloodWait as e:
                        await m.reply_text(str(e))
                        time.sleep(e.x)
                        continue
